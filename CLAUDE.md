...
- **Pipeline progress bar** + **staleness bar**: two stripes above each card — top (8px) = blue fill bar extending left→right across 6 milestones (Applied → 15-min Invited → 15-min Booked → Stretch Requested → Stretch Done → 1-Hr Invited); white/empty = no progress, solid blue = fully through pipeline; Auto-DQ'd = solid red; bottom (5px) = staleness (green→black)
