...
- **Pipeline progress bar** (8px, above each card): blue fill extends left→right across 5 milestones (15-min Invited → 15-min Booked → Stretch Requested → Stretch Done → 1-Hr Invited); white/empty = just applied, solid blue = fully through pipeline; Auto-DQ'd = solid red; staleness is shown as `· Day N` in the card label only
