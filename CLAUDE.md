...
- **Pipeline progress bar** + **staleness bar**: two stripes above each card — top (8px) = 6-segment cumulative progress bar (each completed stage adds a colored segment: gray=Applied, blue=15-min Invited, purple=15-min Booked, orange=Stretch Requested, green=Stretch Done, gold=1-Hr Invited; incomplete segments are light gray; Auto-DQ'd = solid red bar); bottom (5px) = staleness (green→black); a legend renders above the card list in each tab
